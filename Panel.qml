...
Panel {
  id: root
  moduleName: "hendrix.network"
  ipcTarget: "hendrix.network"
  ...
  // Populates the DNS dropdown from `omarchy-dns-ext --list`, which reads
  // ~/.config/omarchy/dns-providers.json. Keeps the last-known list on a
  // parse failure so a transient hiccup doesn't empty the dropdown.
  function updateProviders(raw) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      next.push({ value: entry.id, label: entry.label || entry.id })
    }
    if (next.length > 0) root.dnsProviders = next
  }
    ...
    Column {
      ...
      Column {
        ...
        Dropdown {
          ...
          width: parent.width
          showLabel: false
          value: root.dnsProvider
          options: root.dnsProviders
          foreground: root.bar.foreground
          fontFamily: root.bar.fontFamily
